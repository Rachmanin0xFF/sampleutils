...
public final class PixelMath {
	...
	public static float[][] normalize(float[][] input) {
		...
		if (sum == 0)
			throw new IllegalArgumentException("Input array sums to zero! Cannot normalize!");
		for (int x = 0; x < input.length; x++)
			for (int y = 0; y < input[0].length; y++) {
	...
	/**
	 * Multiplies a kernel by a constant.
	 * @param kernel
	 * @param a
	 * @return kernel*a
	 */
	public static float[][] multKernel(float[][] kernel, float a) {
		float[][] o = new float[kernel.length][kernel[0].length];
	...
	/**
	 * Adds two kernels together entry-wise.
	 * @param kernel1
	 * @param kernel2
	 * @return (kernel1 + kernel2)
	 */
	public static float[][] addKernels(float[][] kernel1, float[][] kernel2) {
		if (kernel1.length % 2 == 0 || kernel1[0].length % 2 == 0 || kernel2.length % 2 == 0
				|| kernel2[0].length % 2 == 0) {
			throw new IllegalArgumentException("Kernel dimensions must be odd numbers!");
		}
		int radiusX1 = (kernel1.length-1)/2;
		int radiusY1 = (kernel1[0].length-1)/2;
		int radiusX2 = (kernel2.length-1)/2;
		int radiusY2 = (kernel2[0].length-1)/2;
		int maxRadiusX = (int)Math.max(radiusX1, radiusX2);
		int maxRadiusY = (int)Math.max(radiusY1, radiusY2);
		float[][] o = new float[maxRadiusX*2 + 1][maxRadiusY*2 + 1];
		for (int x = -maxRadiusX; x <= maxRadiusX; x++) {
			for (int y = -maxRadiusY; y <= maxRadiusY; y++) {
				if(Math.abs(x) <= radiusX1 && Math.abs(y) <= radiusY1) {
					o[x+maxRadiusX][y+maxRadiusY] += kernel1[x+radiusX1][y+radiusY1];
				}
				if(Math.abs(x) <= radiusX2 && Math.abs(y) <= radiusY2) {
					o[x+maxRadiusX][y+maxRadiusY] += kernel2[x+radiusX2][y+radiusY2];
				}
			}
		}
		return o;
	...
	/**
	 * Returns the difference of two Gaussian kernels (with standard deviations specified by stdevpos and stdevneg).
	 * @param stdevpos The positive Gaussian's standard deviation
	 * @param stdevneg The negative Gaussian's standard deviation
	 * @param stdevs_to_include The extent of the Gaussian contained in the kernel,
	 *                          in units of standard deviation.
	 * @return gaussian(stdevpos) - gaussian(stdevneg)
	 */
	public static float[][] differenceOfGaussiansKernel(float stdevpos, float stdevneg, float stdevs_to_include) {
		// The tough part here is that we can't normalize this kernel (it should sum to zero).
		// Because it's a discrete representation of the Gaussian, there's no nice expression
		// to analytically find the normalization factor, so we have to compute it for each Gaussian
		// separately (doing anything else would get a little confusing)
		
		// no reason to not make the smaller kernel go out to the extent of the larger
		boolean posBigger = stdevpos > stdevneg;
		float factor = stdevpos / stdevneg;
		
		float[][] kpos = gaussianKernel(stdevpos, stdevs_to_include * (posBigger ? 1.0f : 1.0f / factor));
		float[][] kneg = multKernel(gaussianKernel(stdevneg, stdevs_to_include * (posBigger ? factor : 1.0f)), -1);
		return addKernels(kpos, kneg);
	}
	 ...
	 * <p>
	 * In addition to standard deviations in the x and y direction, this function
	 * also accepts a Pearson correlation parameter (higher values 'squish' the
	 * Gaussian along the diagonal). Together, these three values are sufficient to
	 * parameterize the space of symmetric covariance matrices (stdevx^2 and
	 * stdevy^2 on the diagonal, rho*stdevx*stdevy on the off-diagonal).
	 * <p>
	 * For axis-aligned distributions (i.e. vertical and horizontal blurs), rho
	 * should be zero.
	 * 
	 * @param stdevx            The standard deviation of the distribution in the x
	 *                          direction.
	 * @param stdevy            The standard deviation of the distribution in the y
	 *                          direction.
	 * @param rho               The Pearson correlation of the Gaussian
	...
	public static float[][] bivariateGaussianKernel(float stdevx, float stdevy, float rho, float stdevs_to_include) {
		if (stdevx <= 0 || stdevy <= 0 || stdevs_to_include <= 0) {
			throw new IllegalArgumentException("Bivariate Gaussian kernel input parameters cannot be <= 0!");
		}
		if (rho >= 1 || rho <= -1) {
			throw new IllegalArgumentException("Bivariate Gaussian kernel correlation must be in range (-1, 1), but it was " + rho);
		}

		// while this kernel is still separable with a change of basis, it is not
		// separable
		...
		for (int x = -sample_radius; x <= sample_radius; x++)
			for (int y = -sample_radius; y <= sample_radius; y++) {
				kernel[x + sample_radius][y + sample_radius] = unnormalizedBivariateGaussian(x, y, stdevx, stdevy, rho);
				sum += kernel[x + sample_radius][y + sample_radius];
			}
		// normalize
	...
	float LoG(float x, float y, float stdev) {
		...
		return -(1.f + xp) * (float) Math.exp(xp) / (PI * stdev * stdev); // NOT NORMALIZED PROPERLY FOR CONSTANT EDGE
																			// DETECTION VALUES WITH VARYING
																			// DEVIATIONS!!!
	}
	...
	// All conversions use the CIE D65 Standard Illuminant unless specified
	// otherwise
	public static Vecf ref_white = new Vecf(95.047f, 100.0f, 108.883f);

	/**
	 * Fairchild-Pirrotta lightness (helmholtz-kohlrausch compensation) of an sRGB
	 * color.
	 * 
	...
	// thanks to
	// https://stackoverflow.com/questions/596216/formula-to-determine-brightness-of-rgb-color/59602392#59602392
	/**
	 * Fairchild-Pirrotta lightness (helmholtz-kohlrausch compensation) of an LCH
	 * color.
	 * 
	 * @param lch The color in LCH space
	 * @return The lightness of the color
	 */
	public static float lightnessFP(Vecf lch) {
		return lch.components[0] + (2.5f - 0.025f * lch.components[0])
				* (0.116f * (float) Math.abs((float) Math.sin(DEG_TO_RAD * ((lch.components[2] - 90.0f) / 2.0f)))
						+ 0.085f)
				* lch.components[1];
	}

	public static Vecf LAB_to_LCH(Vecf c) {
		return new Vecf(c.components[0],
				(float) Math.sqrt(c.components[1] * c.components[1] + c.components[2] * c.components[2]),
				(float) Math.atan2(c.components[2], c.components[1]) * RAD_TO_DEG);
	}
	...
	public static Vecf XYZ_to_LAB(Vecf c) {
		...
		v.components[0] = v.components[0] > 0.008856f ? (float) Math.pow(v.components[0], 1.0f / 3.0f)
				: v.components[0] * 7.787f + 16.0f / 116.0f;
		v.components[1] = v.components[1] > 0.008856f ? (float) Math.pow(v.components[1], 1.0f / 3.0f)
				: v.components[1] * 7.787f + 16.0f / 116.0f;
		v.components[2] = v.components[2] > 0.008856f ? (float) Math.pow(v.components[2], 1.0f / 3.0f)
				: v.components[2] * 7.787f + 16.0f / 116.0f;

		return new Vecf(116.0f * v.components[1] - 16.0f, 500.0f * (v.components[0] - v.components[1]),
				200.0f * (v.components[1] - v.components[2]));
	}

	// LUV is L*u*v*
	public static Vecf XYZ_to_LUV(Vecf c) {
		float v_rp = 9.0f * ref_white.components[1]
				/ (ref_white.components[0] + 15.0f * ref_white.components[1] + 3.0f * ref_white.components[2]);
		float u_rp = 4.0f * ref_white.components[0]
				/ (ref_white.components[0] + 15.0f * ref_white.components[1] + 3.0f * ref_white.components[2]);
	...
	public static Vecf sRGB_to_RGB(Vecf c) {
		return new Vecf(sX_to_X(c.components[0] / 255.0f), sX_to_X(c.components[1] / 255.0f),
	...
	/**
	 * The [delta-E
	 * 2000](http://www2.ece.rochester.edu/~gsharma/ciede2000/ciede2000noteCRNA.pdf)
	 * color distance metric.
	 * 
	 * @param lab1 The first color in sRGB space (0-255)
	 * @param lab2 The second color in sRGB space (0-255)
	 * @return The perceptual distance between the two colors (as approximated by
	 *         the delta-E 2000 standard)
	 */
	public static float DELTA_E_2000_sRGB(Vecf lab1, Vecf lab2) {
		return DELTA_E_2000(XYZ_to_LAB(RGB_to_XYZ(sRGB_to_RGB(lab1))), XYZ_to_LAB(RGB_to_XYZ(sRGB_to_RGB(lab2))));
	}
	...
	public static float DELTA_E_2000(Vecf lab1, Vecf lab2) {
		float C_ab = ((float) Math
				.sqrt(lab1.components[1] * lab1.components[1] + lab1.components[2] * lab1.components[2])
				+ (float) Math.sqrt(lab2.components[1] * lab2.components[1] + lab2.components[2] * lab2.components[2]))
				/ 2.0f;
		float G = 0.5f
				* (1 - (float) Math.sqrt((float) Math.pow(C_ab, 7.0f) / ((float) Math.pow(C_ab, 7f) + 6103515625.0f)));
		...
		float Hp = (float) Math.abs(h1 - h2) > 180f ? (h1 + h2 + 360.0f) / 2.0f : (h1 + h2) / 2.0f;
		...
		float RT = -2f * (float) Math.sqrt((float) Math.pow(Cp, 7f) / ((float) Math.pow(Cp, 7f) + 6103515625.0f)
				* (float) Math.sin(DEG_TO_RAD * 60.0f * (float) Math.exp(-((Hp - 275.0f) * (Hp - 275.0f) / 625.0f))));

		float E00 = (float) Math.sqrt((del_Lp / S_L) * (del_Lp / S_L) + (del_Cp / S_C) * (del_Cp / S_C)
				+ (del_H) / S_H * (del_H) / S_H + RT * del_Cp * del_H / (S_C * S_H));
